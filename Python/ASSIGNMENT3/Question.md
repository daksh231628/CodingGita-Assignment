...
Do not hard-code the values from the test cases.
Test every program with the provided test cases.
For output-prediction questions, write your prediction before running the code.
For debugging questions, identify the reason for the error before correcting it.
Use the requested Python feature where the question specifies it.
Do not use if, elif, else, loops, functions, lists, dictionaries, or other concepts that have not been covered.
Add short comments where the logic is not obvious.
Make sure syntax and indentation are correct.
